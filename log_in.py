...
class Login(QMainWindow):
    def __init__(self):
        ...
        self.btnLogin.clicked.connect(self.loginAdmin)
    ...
    def loginAdmin(self):
        email = self.leUserName.text()
        password = self.lePassword.text()
        
        if len(email) == 0 or len(password) == 0:
            msg = QMessageBox()
            msg.setIcon(QMessageBox.Information)
            msg.setText('Please fill out username and password')
            msg.setWindowTitle("information")
            x = msg.exec_()
        
        else:
            # Check username and password for adminn
